...
#--------question 1------------
curve(dnorm(x,0,0.5),from=-6,to=6,x="oui",n=1000,col=1,main="densité normale pour mu=0 ou 3 et sigma=0.5,1 ou 2")
...
#####################################
abs<-seq(-7,11,0.1)
ord<-dnorm(abs,mu,sigma)
plot(abs,ord,type="l",main="densité de la N(2,9)",xlab="x",ylab="f(x)")
segments(1,0,5,0,col="red")
segments(c(1,5),c(0,0),c(1,5),c(dnorm(1,mu,sigma),dnorm(5,mu,sigma)),col="red")
(bons<-which(abs<=5&abs>=1))
abs[bons]->x.bons
ord[bons]->y.bons
polygon(x = c(min(x.bons), x.bons, max(x.bons)), y = c(0, y.bons, 0), col = "green")
